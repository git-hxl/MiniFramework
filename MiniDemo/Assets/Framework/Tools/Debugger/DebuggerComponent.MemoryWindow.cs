                ...
                GUILayout.BeginVertical("box");
                {
                    m_ScrollPosition = GUILayout.BeginScrollView(m_ScrollPosition);
                    {
                        ...
                        else
                        {
                            ...
                            {
                                GUILayout.Label("<b>" + samples.Count.ToString() + " " + typeName + "s(" + ByteConvert.ByteAutoConvert(sampleSize) + ") obtained at " + sampleTime.ToString("yyyy-MM-dd HH:mm:ss") + ", while " + duplicateSampleCount.ToString() + " " + typeName + "s (" + ByteConvert.ByteAutoConvert(duplicateSampleSize) + ") might be duplicated.</b>");
                            }
                            else

                            {
                                GUILayout.Label("<b>" + samples.Count.ToString() + " " + typeName + "s(" + ByteConvert.ByteAutoConvert(sampleSize) + ") obtained at " + sampleTime.ToString("yyyy-MM-dd HH:mm:ss") + "</b>");
                            }
                            if (samples.Count > 0)
                            {
                            ...
                            for (int i = 0; i < samples.Count; i++)
                            {
                                GUILayout.BeginHorizontal();
                                {
                                    ...
                                    GUILayout.Label(samples[i].HighLight ? "<color=yellow>" + ByteConvert.ByteAutoConvert(samples[i].Size) + "</color>" : ByteConvert.ByteAutoConvert(samples[i].Size), GUILayout.Width(80f));
                                }
                                GUILayout.EndHorizontal();
