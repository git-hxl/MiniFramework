using UnityEngine;
namespace MiniFramework
{
    public class HeartPackComponent : MonoBehaviour
    {
        public enum HeartType
        {
            TCP,
            UDP
        }
        public HeartType Type;
        public int CheckInterval = 5;
        public float PackLatency;

        private float sendTime;
        private float recvTime;

        // Use this for initialization
        void Start()
        {
            MsgManager.Instance.Regist(MsgID.HeartPack, (data) =>
            {
                //接收心跳包 
                recvTime = Time.time;
                PackLatency = recvTime - sendTime;
            });

            switch (Type)
            {
                case HeartType.TCP:
                    TcpHeart();
                    break;
                case HeartType.UDP:
                    UdpHeart();
                    break;
            }
        }

        void TcpHeart()
        {
            this.Sequence().Repeat( -1,CheckInterval, () =>
            {
                if (!TcpClientComponent.Instance.IsConnected)
                {
                    sendTime = 0;
                    recvTime = 0;
                    return;
                }
                if (recvTime - sendTime < 0)
                {
                    Debug.LogError("心跳包接收超时!");
                    TcpClientComponent.Instance.Close();
                    return;
                }
                TcpClientComponent.Instance.Send(MsgID.HeartPack, null);
                sendTime = Time.time;
            }).Begin();
        }

        void UdpHeart()
        {
            this.Sequence().Repeat(-1,CheckInterval, () =>
            {
                if (!UdpClientComponent.Instance.IsActive)
                {
                    sendTime = 0;
                    recvTime = 0;
                    return;
                }
                if (recvTime - sendTime < 0)
                {
                    Debug.LogError("心跳包接收超时!");
                    UdpClientComponent.Instance.Close();
                    return;
                }
                UdpClientComponent.Instance.Send(MsgID.HeartPack, null);
                sendTime = Time.time;
            }).Begin();
        }
    }

}